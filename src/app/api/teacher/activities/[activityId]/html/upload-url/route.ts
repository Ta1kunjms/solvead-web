...
export async function GET(request: NextRequest, { params }: { params: Promise<Params> }) {
  ...
  if (activity.created_by && activity.created_by !== user.id) {
    return NextResponse.json({ error: "Forbidden: You did not create this activity" }, { status: 403 });
  }
  ...
  const path = `activities/${activityId}/activity.html`;
